...
func (db *Database) Close() error {
	...
	if db.quitChan != nil {
		...
		db.quitChan <- errc
		if err := <-errc; err != nil {
			log.Error("Metrics collection failed, err: %v")
		}
		db.quitChan = nil
	}
	return db.db.Close()
...
func (db *Database) meter(refresh time.Duration) {
	...
	for i := 1; errc == nil && merr == nil; i++ {
		...
		if err != nil {
			log.Error("Failed to read database stats, err: %v")
			merr = err
			continue
		}
		...
		for _, line := range lines {
			...
			for idx, counter := range parts[3:] {
				...
				if err != nil {
					log.Error("Compaction entry parsing failed, err: %v", err)
					merr = err
					continue
				}
		...
		if err != nil {
			log.Error("Failed to parse delay duration, err: %v", err)
			merr = err
			continue
		}
		...
		if len(parts) < 2 {
			log.Error("Bad syntax of ioStats, ioStats: %v", ioStats)
			merr = fmt.Errorf("bad syntax of ioStats %s", ioStats)
			continue
		}
		if n, err := fmt.Sscanf(parts[0], "Read(MB):%f", &nRead); n != 1 || err != nil {
			log.Error("Bad syntax of read entry, entry: %v", parts[0])
			merr = err
			continue
		}
		if n, err := fmt.Sscanf(parts[1], "Write(MB):%f", &nWrite); n != 1 || err != nil {
			log.Error("Bad syntax of write entry, entry: %v", parts[1])
			merr = err
			continue
		}
